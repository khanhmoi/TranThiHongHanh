    ...
    public partial class frmQLDiem : Form
    {
      ...
      /*      cboLop.Items.Clear();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string select1 = "Select MaSV from tblSINH_VIEN where MaSV=N'" + cboMaSV.Text + "' and Hoten=N'"+ txtHoTen.Text +"' ";
            SqlCommand cmd1 = new SqlCommand(select1, conn);
            SqlDataReader reader1 = cmd1.ExecuteReader();
            errorProvider1.Clear();
            if (cboMaSV.Text == "")
            {
                errorProvider1.SetError(cboMaSV, "Mã sinh viên không để trống!");
                cboMaSV.Focus();
            }
            else if (cboMaSV.Text == dgrDiem.CurrentRow.Cells[0].Value.ToString() && cboMonHoc.Text == dgrDiem.CurrentRow.Cells[3].Value.ToString())
            {
                {
            ...
            else if (reader1.Read())
            {
                ...
                "Values('" + cboMaSV.Text + "',N'" + txtHoTen.Text + "','" + cboLop.Text + "',N'" + cboMonHoc.Text + "','" + txtDiemThi1.Text + "','" +
        ...
        private void cboLop_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            cboMaSV.Items.Clear();
            cboMaSV.Text = "";
            string select = "Select MaSV from tblSINH_VIEN where tblSINH_VIEN.MaLop = '" + cboLop.Text + "'";
            SqlCommand cmd = new SqlCommand(select, conn);
            SqlDataReader reader = cmd.ExecuteReader();
            //Add vao cboLop
            while (reader.Read())
            {
                ...
                cboMaSV.Items.Add(reader.GetString(0));
            }
            //Tra tai nguyen 
            reader.Dispose();
            cmd.Dispose();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn muốn xóa ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                string delete = "delete from tblKET_QUA where MaSV='" + cboMaSV.Text + "' and MaMon='" + cboMonHoc.Text + "' ";
        ...
        {
            cboMaSV.Text = dgrDiem.CurrentRow.Cells[0].Value.ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string update = "Update tblKET_QUA Set HoTen=N'" + txtHoTen.Text + "',MaMon=N'" +cboMonHoc.Text + "',MaLop='" + 
                                ...
                                txtGhiChu.Text + "' where MaSV='" + cboMaSV.Text + "' and MaMon=N'" + cboMonHoc.Text + "'";
        ...
        private void txtDiemThi1_TextChanged(object sender, EventArgs e )
        {
            ...
            if (DIEMTK <= 4.5)
            {
                this.txtGhiChu.Text = "Trượt môn";
            }
            else 
            {
                this.txtGhiChu.Text  = "Qua môn";
            }
        ...
        {
            string select = "Select HoTen from tblSINH_VIEN where tblSINH_VIEN.MaSV = '" + cboMaSV.Text + "'";
            SqlCommand cmd = new SqlCommand(select, conn);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.HasRows)
            {
                reader.Read();
                txtHoTen.Text = reader.GetString(0).ToString();
            }
            reader.Dispose();
            cmd.Dispose();
        }
